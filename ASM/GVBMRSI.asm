         TITLE 'GVBMRSI - INITIALIZATION FOR SUMMARY ROUTINE'
**********************************************************************
*
* (C) COPYRIGHT IBM CORPORATION 2000, 2023.
...
*
         larl  r11,GVBMRSI        SET  static area base
         USING (GVBMRSI,code),R11
...
newsa    using saver,r1
         ...
         TIME  STCK,(R2),LINKAGE=SYSTEM,DATETYPE=YYYYMMDD,             +
               MF=(E,(R5))
*  Call SORT - SORTE35 will be called with sorted rec next               
...
*                                                                     *
*        (THIS CODE EXECUTES AS AN EXTENSION OF GVBMRSM)              *
*                                                                     *
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
*
         USING WORKAREA,R13
...
*
* Read SORT parms that are in SYSIN
*
         L     R2,SYSINDCB
         USING IHADCB,R2
         LAY   R14,STATOPEN
...
*
GVBMRSM  DC    CL8'GVBMRSM '      MAIN PROGRAM NAME
...
***********************************************************************
         ...
         using ihadcb,r2
         if    TM,dcboflgs,dcbofopn,z      Failed to open ?
           WTO 'GVBMRSM - UNABLE TO OPEN LOG FILE (MR88LOG)'
           ABEND 020
         else
***********************************************************************
...
***********************************************************************
         ...
         WTO   'GVBMRSM - UNABLE TO OPEN CONTROL REPORT FILE (MR88RPT)'
         ABEND 020
*
...
***********************************************************************
* Check parms passed in MR88PARM file                                 *
***********************************************************************
         L     R3,PARMDADR
         USING PARMDATA,R3
***********************************************************************
*  Set "NO HEADER RECORDS" flag (always sorted)                       *
***********************************************************************
...
*
EXTDECBL EQU   *               Top of DECB loop
         LA    R14,MDLREADL+4(,R3)   LOAD ADDRESS OF   NEXT DECB
...
*
* on return SORT has been called and we have been called back via 
* the exit E35 with a sorted record         
*
...
*
         ...
         JP    SORTOUT            NO  - PASS    RECORD  TO  "GVBMRSM"
*
...
*
* savesort save areas will return us to where the EXTRCHKA was called
         using saver,savesort
         LM    R14,R12,savgrs14   RESTORE "GVBMRSM"  REGISTERS
         BSM   0,R14
*
*        DROP  R10
         DROP  R13
...
*
         ...
         using saver,savesort
*
E35RETRN STM   R14,R12,savgrs14        SAVE "GVBMRSM" REGISTER CONTENTS
*
